...
def every(colors, output_dir=CACHE_DIR):
    ...
    create_obsidian_css(colors, output_dir)  
...
def color(colors, export_type, output_file=None):
    ...
    if os.path.isfile(template_file):
        ...
        create_obsidian_css(all_colors)
...
#                                                           \\//
#                                                            \/
OBSIDIAN_SNIPPETS_DIR = os.path.expanduser("~/MyObsidianNotes/.obsidian/snippets/")

def create_obsidian_css(colors, output_dir=CACHE_DIR):
    obsidian_css_path = os.path.join(OBSIDIAN_SNIPPETS_DIR, "obsidian.css")

    with open(obsidian_css_path, 'w') as css_file:
        css_file.write("/* CSS variables\n   Generated by 'wal' */\n")
        
        css_file.write(":root {\n")
        for i in range(16): 
            color_var = f"--color{i}: {colors[f'color{i}']};\n"
            css_file.write(f"    {color_var}")
        css_file.write("}\n\n")
        
        css_file.write(".theme-dark {\n")
        css_file.write("    --color-base-00: var(--color0); /* Фон */\n")
        css_file.write("    --color-base-20: none;\n")
        css_file.write("    --color-base-30: none;\n")
        css_file.write("}\n")

        css_file.write("body.theme-dark{\n")
        css_file.write("    color: var(--color7);")
        css_file.write("}\n")

        css_file.write(".cm-header{\n")
        css_file.write("    color: var(--color3);\n")
        css_file.write("}\n")
        
        css_file.write(".workspace-tab-header-container {\n")
        css_file.write("background-color: var(--background);\n")
        css_file.write("}\n")

        
        css_file.write(".side-dock-ribbon-action {\n")
        css_file.write("color: var(--color9);\n")
        css_file.write("}\n")

    logging.info("Generated obsidian.css at %s", obsidian_css_path)
