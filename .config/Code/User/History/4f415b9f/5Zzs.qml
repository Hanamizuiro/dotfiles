...
Item {
    ...
    // Minimal stub to satisfy ControlCenter usage:
    // - should support anchors.fill and z
    // - provide show(device) and hide()
    // - emit codecSelected(deviceAddress, codecName)
    ...
    function show(device) {
        // The real implementation shows a UI and lets the user pick a codec.
        // This stub simply records the device and becomes visible.
        try {
            root._device = device
        } catch (e) {
            root._device = null
        ...
        root.visible = true
        // No interactive UI in the stub; do nothing else.
    }
    ...
    // Helper to programmatically emit a codecSelected event for testing or
    // downstream code that expects a codec to be selected. Call like:
    // bluetoothCodecSelector.emitCodecSelected('AA:BB:CC:DD:EE:FF', 'LDAC')
    function emitCodecSelected(deviceAddress, codecName) {
        codecSelected(deviceAddress, codecName)
    }

    // Keep an internal device reference if provided
