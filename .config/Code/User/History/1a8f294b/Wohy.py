...
# status code
try:
    status_code = html_data("#regionHeader").attr("class").split(" ")[2].split("-")[1]
except (AttributeError, IndexError):
    # Fallback for different class structure
    try:
        class_str = html_data("div[data-testid='CurrentConditions--CurrentConditions--1_1_1']").attr("class")
        if class_str:
            status_code = class_str.split("CurrentConditions--CurrentConditions--")[1].split(" ")[0]
        else:
            status_code = "default"
    except (AttributeError, IndexError):
        status_code = "default"
