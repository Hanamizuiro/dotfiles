...
# status code - Fixed to handle missing element
try:
    status_code = html_data("#regionHeader").attr("class").split(" ")[2].split("-")[2]
except (IndexError, AttributeError, TypeError):
    # Fallback: determine status code from status text
    status_lower = status.lower()
    if 'sunny' in status_lower or 'clear' in status_lower:
        status_code = 'sunnyDay'
    elif 'rain' in status_lower or 'drizzle' in status_lower or 'shower' in status_lower:
        status_code = 'rainyDay'
    elif 'cloud' in status_lower or 'overcast' in status_lower:
        status_code = 'cloudyFoggyDay'
    elif 'snow' in status_lower or 'ice' in status_lower or 'freez' in status_lower:
        status_code = 'snowyIcyDay'
    elif 'fog' in status_lower or 'mist' in status_lower:
        status_code = 'cloudyFoggyDay'
    elif 'storm' in status_lower or 'thunder' in status_lower:
        status_code = 'severe'
    else:
        status_code = 'default'
