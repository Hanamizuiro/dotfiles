...
apply_image_wallpaper() {
  ...
  # Update quickshell/rofi wallpaper file so other apps (like QuickShell) can read it
  mkdir -p "$HOME/.config/rofi"
  # Copy the image into a stable location so QML Image can load it via a constant path
  cp -f "$image_path" "$HOME/.config/rofi/.current_wallpaper" 2>/dev/null || true
  # Also record the selected absolute path for other scripts
  if [[ -n "$wallpaper_current" ]]; then
    mkdir -p "$(dirname "$wallpaper_current")"
    printf "%s" "$image_path" > "$wallpaper_current" || true
  fi
    # Also write QuickShell-friendly files: path and timestamp
    mkdir -p "$HOME/.config/quickshell"
    printf "%s" "$image_path" > "$HOME/.config/quickshell/current_wallpaper_path" || true
    date +%s > "$HOME/.config/quickshell/current_wallpaper_ts" || true

  # Integrate the theme/color update commands here
  wal -i "$image_path" 

  # symoblik link for vencord
...
apply_video_wallpaper() {
  ...
  # For video wallpapers, write a thumbnail to rofi current wallpaper so image viewers can show something
  mkdir -p "$HOME/.config/rofi"
  if command -v ffmpeg >/dev/null 2>&1; then
    ffmpeg -v error -y -i "$video_path" -ss 00:00:01.000 -vframes 1 "$HOME/.config/rofi/.current_wallpaper" >/dev/null 2>&1 || true
  else
    # Fallback: write the video path to wallpaper_current for other tools
    if [[ -n "$wallpaper_current" ]]; then
      mkdir -p "$(dirname "$wallpaper_current")"
      printf "%s" "$video_path" > "$wallpaper_current" || true
    fi
    fi
    # Also write QuickShell-friendly files for video
    mkdir -p "$HOME/.config/quickshell"
    printf "%s" "$video_path" > "$HOME/.config/quickshell/current_wallpaper_path" || true
    date +%s > "$HOME/.config/quickshell/current_wallpaper_ts" || true
  fi
}
